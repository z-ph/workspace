...
def ensure_result_dir():
    """确保result文件夹及其子文件夹存在"""
    result_dir = 'result/random_forest_with_smote'
...
def apply_smote(X, y):
    ...
    返回:
        ...
        smote_applied: 是否应用了SMOTE
    """
    # 检查目标变量是否有多个类别
    unique_classes = np.unique(y)
    
    if len(unique_classes) < 2:
        print(f"警告: 目标变量只有 {len(unique_classes)} 个类别，无法应用SMOTE过采样。")
    ...
    if any(count < 2 for count in class_counts):
        print(f"警告: 某些类别的样本数少于2个，无法应用SMOTE过采样。")
    ...
    # 打印过采样前后的样本分布
    print(f"过采样前 - 正常样本数: {sum(y == 0)}, 异常样本数: {sum(y == 1)}")
    print(f"过采样后 - 正常样本数: {sum(y_resampled == 0)}, 异常样本数: {sum(y_resampled == 1)}")
...
# 生成分析报告
def generate_report(accuracy, report, cv_scores, feature_importance_df, X_resampled, y_resampled, result_dir, smote_applied):
    """
    生成分析报告
    参数:
        accuracy: 准确率
        ...
        smote_applied: 是否应用了SMOTE
    """
    report_path = os.path.join(result_dir, 'random_forest_with_smote_analysis.md')
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if smote_applied:
            f.write('- 已应用SMOTE过采样处理\n')
        else:
            f.write('- 未应用SMOTE过采样（因目标变量类别不足或样本数过少）\n')
        f.write('\n')
...
def main():
    ...
    # 生成分析报告
    generate_report(accuracy, report, cv_scores, feature_importance_df, X_resampled, y_resampled, result_dir, smote_applied)
